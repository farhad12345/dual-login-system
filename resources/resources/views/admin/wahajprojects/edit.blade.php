<x-app-layout>
    ...
    <div class="container mt-5 back1">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <form action="{{ route('admin.wahajprojects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="service" id="service" class="form-control  @error('service') is-invalid @enderror">
                                ...
                                <option value="record_number" {{ old('record_number', $project->service) === 'record_number' ? 'selected' : '' }}>بيع</option>
                                <option value="buying" {{ old('buying', $project->service) === 'buying' ? 'selected' : '' }}>شراء</option>
                                <option value="investement" {{ old('investement', $project->service) === 'investement' ? 'selected' : '' }}>استثمار</option>
                            </select>
                            @error('service')
                                <div class="invalid-feedback">
                                    {{ $message }}
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="property_type" id="property_type" class="form-control @error('property_type') is-invalid @enderror" onchange="toggleOtherField()">
                                <option value="">Select </option>
                                <option value="land" {{ old('land', $project->property_type) == 'land' ? 'selected' : '' }}>ارض</option>
                                <option value="building" {{ old('building', $project->property_type) == 'building' ? 'selected' : '' }}>عمارة</option>
                                <option value="villa" {{ old('villa', $project->property_type) == 'villa' ? 'selected' : '' }}>فله</option>
                                <option value="apartment" {{ old('apartment', $project->property_type) == 'apartment' ? 'selected' : '' }}>شقة</option>
                                <option value="hotel" {{ old('hotel', $project->property_type) == 'hotel' ? 'selected' : '' }}>فندق</option>
                                <option value="mall" {{ old('mall', $project->property_type) == 'mall' ? 'selected' : '' }}>مول</option>
                                <option value="indutrial_land" {{ old('indutrial_land', $project->property_type) == 'indutrial_land' ? 'selected' : '' }}>ارض صناعية</option>
                                <option value="commertial_warehous" {{ old('commertial_warehous', $project->property_type) == 'commertial_warehous' ? 'selected' : '' }}>مستودعات تجارية</option>
                                <option value="gas_stattion" {{ old('gas_stattion', $project->property_type) == 'gas_stattion' ? 'selected' : '' }}>محطات وقود</option>
                                <option value="failure" {{ old('failure', $project->property_type) == 'failure' ? 'selected' : '' }}>قصور</option>
                                <option value="farm" {{ old('farm', $project->property_type) == 'farm' ? 'selected' : '' }}>مزارع</option>
