...
class DashboardController extends Controller
{
    ...
    ->orderBy('wahaj_watan_details.id', 'desc') // Order by latest record
     ->paginate(10);
        ...
        // Query for future or ongoing projects
        $mawayeedprojects = MawayeedProject::with('employee')
            ->select(
                'mayeed_projects.*',
                'mawayeed_users.id as user_id',
            ...
            ->leftJoin('mawayeed_users', 'mayeed_projects.employee_id', '=', 'mawayeed_users.id')
            ->whereRaw("STR_TO_DATE(mayeed_projects.date, '%d-%m-%Y') >= STR_TO_DATE(?, '%d-%m-%Y')", [$currentDate]) // Future or ongoing
            ->orderByRaw("STR_TO_DATE(mayeed_projects.date, '%d-%m-%Y') ASC") // Order by ascending date
            ->paginate(10);
    // Project Statuses
    ...
    public function ViewProjects($id)
    {
        ...
        $name=$user->name;
        return view('admin.projects.details',compact('projects','name'));
    }
    ...
    public function UserUpdate(Request $request, $id)
{
    ...
    // Update password if it's provided (nullable check)
    ...
    return redirect()->route('admin.users.lists')->with('success', 'User updated successfully');
}
    ...
    public function saveReason(Request $request)
    {
        ...
        return redirect()->route('dashboard')->with('success', 'تم حفظ السبب بنجاح وإرسال إشعار إلى الموظف!');
    }
    public function GetMaveedPrevoisuListData(Request $request)
    {
        try {
            ...
            $projectsQuery = MawayeedProject::with('employee')
                ...
                ->leftJoin('mawayeed_users', 'mayeed_projects.employee_id', '=', 'mawayeed_users.id')
                ->whereRaw("STR_TO_DATE(mayeed_projects.date, '%d-%m-%Y') < STR_TO_DATE(?, '%d-%m-%Y')", [$currentDate]);
            ...
            $projects = $projectsQuery
                ->orderByRaw("STR_TO_DATE(mayeed_projects.date, '%d-%m-%Y') DESC") // Sort by descending date
                ->paginate(10);
